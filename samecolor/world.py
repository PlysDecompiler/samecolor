...
class Entity(object):
    ...
    def purge_silently(self):
        self.scene.purge_silently(self)
...
class Scene(object):
    ...
    def add(self, entity, name = None):
        """
        If name is given the entity is added to the scene AND added to the dynamic_members which are accesible via attribute notation
        """
        if hasattr(entity, "scene"):
            ...
            if not isinstance(entity, Entity):
                ...
                print "You either have to implement a purge function yourself and call that or manually purge it from the scene."
    ...
    def iter_instances(self, type_of_instances):
        try:
            ...
            #raise ValueError("This scene does not contain any instances of type '%s.%s'" % (type_of_instances.__module__, type_of_instances.__name__))
            return [] 
